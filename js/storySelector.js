...
StorySelector.prototype.wrangleData = function(){
    ...
    vis.stories.forEach(story => {
        story.percentMale = story.maleCount / (story.maleCount+story.neutralCount+story.femaleCount);
        story.percentFemale = story.femaleCount / (story.maleCount+story.neutralCount+story.femaleCount);
    });

    vis.stories.sort((a, b) => {
            return (b.percentMale + (1-b.percentFemale))/2 - (a.percentMale + (1-a.percentFemale))/2;
    });
...
StorySelector.prototype.updateVis = function(){
    ...
    let stories = vis.svg.selectAll(".title")
        .data(vis.stories);
    ...
    stories.enter()
        ...
        .classed("active", (d, i) => {
            if (d.title=="All Stories") {
                vis.svg.append("text").text("📘").attr("class", "bookIcon").attr("y", i*18 + 20);
                return true;
            }
            else {
                return false;
    ...
    function selectStory(e) {
        ...
        story.classed("active", true).transition().duration(300).ease(d3.easeBackOut).attr("x", 21);

        vis.svg.append("text").text("📘").attr("class", "bookIcon").attr("y", story.attr("y"));
