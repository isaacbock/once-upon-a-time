/*
 * DescriptionRatio - Object constructor function
...
DescriptionRatio = function (_parentElement, _words, _colors) {
...
DescriptionRatio.prototype.initVis = function () {
	var vis = this;
	...
	// Scale
	vis.xScale = d3.scaleLinear().domain([0, 1]).range([0, vis.width]);
...
DescriptionRatio.prototype.wrangleData = function () {
	var vis = this;
	...
	vis.malePercentage =
		vis.maleCount / (vis.maleCount + vis.neutralCount + vis.femaleCount);
	vis.neutralPercentage =
		vis.neutralCount / (vis.maleCount + vis.neutralCount + vis.femaleCount);
	vis.femalePercentage =
		vis.femaleCount / (vis.maleCount + vis.neutralCount + vis.femaleCount);
	...
	vis.cumulativePercentages = [
		...
		{
			startingPoint: vis.malePercentage + vis.neutralPercentage,
			width: vis.femalePercentage,
		},
	];
	...
	vis.countData = [
		...
		{
			gender: "neutral",
			count: vis.neutralCount,
			percentage: vis.neutralPercentage,
		},
		{
			gender: "female",
			count: vis.femaleCount,
			percentage: vis.femalePercentage,
		},
	];
...
DescriptionRatio.prototype.updateVis = function () {
	var vis = this;

	// plot DescriptionRatio
	let ratioBars = vis.svg
		.selectAll(".ratio.description")
	...
	ratioBars
		...
		.classed("ratio description", true)
	...
	function mouseover(e) {
		...
		if (e.target.classList.contains("male")) {
			...
			// Tooltip setup adapted from Yan Holtz (2018). Scatterplot with tooltip in d3.js. D3.js Graph Gallery. Retrieved Nov. 3, 2020 from https://www.d3-graph-gallery.com/graph/scatter_tooltip.html
			vis.tooltip.html("<h3>" + count + " male descriptors" + "</h3>");
			...
			vis.tooltip.html(
				"<h3>" + count + " gender-neutral descriptors" + "</h3>"
			);
			...
			vis.tooltip.html("<h3>" + count + " female descriptors" + "</h3>");
		}
	}
