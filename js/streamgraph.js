...
Streamgraph.prototype.wrangleData = function(){
    ...
    vis.countData.forEach(gender => { gender.count = 0; });
    ...
    vis.binSentences = [];
    vis.binSentences.push({bin: 0, maleCount: 0, neutralCount: 0, femaleCount: 0});
    let bins = 20;
    ...
    while (i<1) {
        ...
        let femaleCount = 0;
        vis.filteredSentences.forEach(sentence => {
            if (sentence.sentencePercentage>i-binWidth/2 && sentence.sentencePercentage<=i+binWidth/2) {
                maleCount += sentence.maleCount;
                neutralCount += sentence.neutralCount;
                femaleCount += sentence.femaleCount;
            }
        });
        vis.binSentences.push({bin: i, maleCount: maleCount, neutralCount: neutralCount, femaleCount: femaleCount});
        i += binWidth;
    }
    vis.binSentences.push({bin: 1, maleCount: 0, neutralCount: 0, femaleCount: 0});
...
Streamgraph.prototype.updateVis = function(){
	...
	// Scales and axes
    vis.xScale
        .domain([0, d3.max(vis.binSentences, d => d.bin)]).nice()
        .range([0,vis.width]);

    vis.yScale
        .domain([-1*d3.max(vis.binSentences, d => d.maleCount + d.neutralCount + d.femaleCount)/2, d3.max(vis.binSentences, d => d.maleCount + d.neutralCount + d.femaleCount)/2])
        .range([vis.height,0]);

    vis.yAxisScale
        .domain([0, d3.max(vis.binSentences, d => d.maleCount + d.neutralCount + d.femaleCount)]).nice()
        .range([vis.height,0]);

    // modify bin data into stack for streamgraph
    let stack = d3.stack().offset(d3.stackOffsetSilhouette).order(d3.default).keys(["femaleCount", "neutralCount", "maleCount"]);
    let stackedData = stack(vis.binSentences);
    // convert data to streamgraph path areas
    let area = d3.area()
        .x(function(d) { return vis.xScale(d.data.bin); })
    ...
    percentages.enter().append("text")
        .text(d => {
            ...
            let totalCount = vis.countData[0].count + vis.countData[1].count + vis.countData[2].count;
            if (d.gender=="male") {
    ...
    function mouseover(e) {
        ...
        if (e.target.classList.contains("male")) {
            ...
            vis.tooltip
                .html(
                    "<h3>"+count+" gender-neutral references"+"</h3>"
                );
        }
