...
ForceLayout.prototype.wrangleData = function(){
    ...
    vis.filteredWords.forEach( word => {
        if (!vis.comparisonData.some(existingWord => existingWord.word===word.word && existingWord.partOfSpeech===word.partOfSpeech)) {
            vis.comparisonData.push({word: word.word, partOfSpeech: word.partOfSpeech, maleCount: 0, neutralCount: 0, femaleCount: 0});
        }
        let existingWord = vis.comparisonData.filter(existingWord => existingWord.word===word.word && existingWord.partOfSpeech===word.partOfSpeech)[0];
        if (word.gender=="male") {
    ...
    vis.comparisonData.forEach( word => {
        word.percentMale = word.maleCount / (word.maleCount+word.neutralCount+word.femaleCount);
        word.percentNeutral = word.neutralCount / (word.maleCount+word.neutralCount+word.femaleCount);
        word.percentFemale = word.femaleCount / (word.maleCount+word.neutralCount+word.femaleCount);
    });

    vis.comparisonData.sort((a,b) => Math.abs(((b.percentMale + 1-b.percentFemale)/2)-.5) - Math.abs(((a.percentMale + 1-a.percentFemale)/2)-.5));
    vis.comparisonData = vis.comparisonData.slice(0, 500);
    vis.comparisonData.sort((a,b) => (b.maleCount+b.neutralCount+b.femaleCount) - (a.maleCount+a.neutralCount+a.femaleCount));
...
ForceLayout.prototype.updateVis = function(){
    ...
    if (vis.story=="all") {
        vis.circleScale
            .domain([
                d3.min(vis.comparisonData, d => d.maleCount+d.neutralCount+d.femaleCount),
                d3.max(vis.comparisonData, d => d.maleCount+d.neutralCount+d.femaleCount)
            ])
            .range([4, 60]);
        ...
        vis.simulation = d3.forceSimulation(vis.comparisonData)
    ...
    function ticked() {
        var bubbles = vis.svg.selectAll('.bubble')
          .data(vis.comparisonData)
      
        bubbles.enter().append('circle')
            .attr('r', d => vis.circleScale(d.maleCount+d.neutralCount+d.femaleCount))
            .attr("fill", d => vis.colorScale((d.percentMale + 1-d.percentFemale)/2))
            .attr("stroke", "grey")
            .attr("class", "bubble")
    ...
    function mouseover(e) {
        ...
        vis.tooltip
            ...
            .html(
                "<h3>"+word+"</h3>"+
                "<hr>"+
                "<p>"+maleCount+" male uses ("+malePercent+")</p>"+
                "<p>"+neutralCount+" gender-neutral uses ("+neutralPercent+")</p>"+
                "<p>"+femaleCount+" female uses ("+femalePercent+")</p>"
            );
